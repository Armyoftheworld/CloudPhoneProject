...
public class ControlMessageReader {
    ...
    public ControlMessageProto.ControlMessage newNext() {
        if (!buffer.hasRemaining()) {
            return null;
        }
        int savedPosition = buffer.position();
        ControlMessageProto.ControlMessage msg = null;
        if (buffer.getShort() == SocketConstants.DATA_BEGIN) {
            int len = buffer.getInt();
            if (buffer.remaining() >= len) {
                byte[] data = new byte[len];
                try {
                    buffer.get(data);
                    msg = ControlMessageProto.ControlMessage.parseFrom(data);
