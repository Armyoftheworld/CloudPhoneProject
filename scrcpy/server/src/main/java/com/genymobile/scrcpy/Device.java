...
public final class Device {
    ...
    public Point newGetPhysicalPoint(ControlMessageProto.ControlMessage msg) {
        // it hides the field on purpose, to read it with a lock
        @SuppressWarnings("checkstyle:HiddenField")
        ScreenInfo screenInfo = getScreenInfo(); // read with synchronization
        Size videoSize = screenInfo.getVideoSize();
        if (videoSize.getHeight() != msg.getHeight() || videoSize.getWidth() != msg.getWidth()) {
            // The client sends a click relative to a video with wrong dimensions,
            // the device may have been rotated since the event was generated, so ignore the event
            return null;
        }
        Rect contentRect = screenInfo.getContentRect();
        int scaledX = contentRect.left + msg.getX() * contentRect.width() / videoSize.getWidth();
        int scaledY = contentRect.top + msg.getY() * contentRect.height() / videoSize.getHeight();
        return new Point(scaledX, scaledY);
    }
